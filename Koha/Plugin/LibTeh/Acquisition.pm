...
BEGIN {
    ...
    my $lib_dir    = "$plugin_dir/lib";
    
    unless (grep { $_ eq $lib_dir } @INC) {
        unshift @INC, $lib_dir;
    }
}
...
sub new {
    ...
    #$self->{db} = Koha::Plugin::LibTeh::Acquisition::lib::DB->new();
...
sub dbh {
    my ($self) = @_;
    return Koha::Plugin::LibTeh::Acquisition::lib::DB->new();
}

# Інсталяція: створення таблиць
sub install {
    my ( $self, $args ) = @_;
    $self->dbh->init_schema();
...
sub uninstall {
    my ( $self, $args ) = @_;
    my $dbh = Koha::Plugin::LibTeh::Acquisition::lib::DB->new();
    $dbh->destruct_schema();
...
sub intranet_head {
    ...
    my ( $self, $args ) = @_;
    #return "Koha::Plugin::LibTeh::Acquisition::intranet_head";  
}
...
sub tool {
    ...
    if ($op eq 'kso_in' or $op eq 'cud-save_kso1' or $op eq 'get_kso1' or $op eq 'get_kso1_items') {
        $self->page_kso_in();
    } elsif ( $op eq 'kso_out' ) {
        $self->page_kso_out();
...
sub page_kso_in {
    ...
    if ($op eq 'get_kso1_items') {
        ...
        my $items = $dbh->selectall_arrayref(
            ...
            "SELECT 
                i.itemnumber,
                i.barcode AS inv_number,
                i.price AS price,
                b.biblionumber,
                b.title,
                b.author,
                ExtractValue(bi.metadata, '//datafield[\@tag=\"245\"]/subfield[\@code=\"n\"]') AS volume_number,
                ExtractValue(bi.metadata, '//datafield[\@tag=\"245\"]/subfield[\@code=\"p\"]') AS volume_title,
                ExtractValue(bi.metadata, '//datafield[\@tag=\"260\" or \@tag=\"264\"]/subfield[\@code=\"a\"]') AS place,
                ExtractValue(bi.metadata, '//datafield[\@tag=\"260\" or \@tag=\"264\"]/subfield[\@code=\"b\"]') AS publisher,
                b.copyrightdate AS pub_year
            FROM items i
            JOIN biblio b ON i.biblionumber = b.biblionumber
            JOIN biblio_metadata bi ON b.biblionumber = bi.biblionumber
            WHERE ExtractValue(i.more_subfields_xml, '//datafield[\@tag=\"999\"]/subfield[\@code=\"A\"]') = ?" ,
    ...
    if ($op eq 'cud-save_kso1') {
        $self->dbh->save_kso1({
            id               => $cgi->param('kso_id'),
            reg_date         => $cgi->param('reg_date'),
            doc_num          => $cgi->param('doc_num'),
            doc_num_supplier => $cgi->param('doc_num_supplier'),
            supplier_id      => $cgi->param('supplier_id') || undef,
            finance_source   => $cgi->param('finance_source'),
            total_amount     => $cgi->param('total_amount') || 0,
            titles_count     => $cgi->param('titles_count') || 0,
            items_count      => $cgi->param('items_count') || 0,
            is_completed     => $cgi->param('is_completed') ? 1 : 0,
...
sub page_kso_movement {
    ...
    if ($op eq 'delete_kso_movement') {
        my $id = $cgi->param('id');
        $dbh->do("DELETE FROM libteh_kso3 WHERE id = ?", undef, $id);
    ...
    if ($selected_base_id) {
        my $base = $dbh->selectrow_hashref("SELECT end_items, end_amount FROM libteh_kso3 WHERE id = ?", undef, $selected_base_id);
        if ($base) {
    ...
    my $previous_periods = $dbh->selectall_arrayref("SELECT * FROM libteh_kso3 ORDER BY id DESC", { Slice => {} });
